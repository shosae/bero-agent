...
from app.modules.robot.services.validator_rule_service import (
...
4. JSON 구조/스키마/마지막 basecamp 복귀는
...
4) 순서
   ...
   - 단, basecamp로 복귀는 항상 마지막이므로 순서 예외로 허용.
...
_LLM_LOG_PATH = artifacts_dir() / "plan_validator_llm.log"
...
class LLMValidatorService:
    ...
    def validate(
            ...
            *,
            ...
            try:
                # 미리 리스트를 문자열로 변환
                actions_list = ", ".join([a.get("name", "") for a in self.actions])
                locations_list = ", ".join([l.get("id", "") for l in self.locations])
