...
4. **미션 종료 하드 규칙:** 사용자가 "거기 계속 있어라", "대기해", "머물러 있어", "기다려" 등 **명시적으로 특정 위치에 계속 머무르라고 지시하는 표현**을 사용하지 않은 이상,
   모든 PLAN은 **반드시 basecamp로 복귀**해야 한다. (마지막 step이 navigate(basecamp)가 아니면 잘못된 PLAN이다.)
5. **순수 이동 미션 처리:** 사용자가 특정 위치만 언급하고
   ...
   - 복귀는 전역 규칙에 따라 `navigate(target="basecamp")`로 마무리한다.
...
- **기본 원칙:**
  - 사용자가 "거기 계속 있어라", "그 자리에서 대기해", "그 위치에서 계속 봐줘",
    ...
    모든 미션은 **반드시 basecamp로 복귀해야 한다.**

- 따라서 plan 배열의 **마지막 step은 항상** 아래와 같아야 한다.

  { "action": "navigate", "params": { "target": "basecamp" } }

- 사용자가 "와", "돌아와", "하고 와", "갔다 와", "해주고 와" 등
  복귀를 암시하는 표현을 쓰든 쓰지 않든,
  **예외적 지시(계속 있어, 대기해 등)가 없다면** 위 step을 반드시 마지막에 넣어야 한다.
...
- 예외:
  ...
  - 이 예외가 아닌데 마지막 step이 navigate(basecamp)가 아니라면,
    그 PLAN은 **잘못된 PLAN**이다.
...
- 이 경우 PLAN은 다음과 같이 구성한다.
  ...
  2) 마지막에는 전역 규칙에 따라
     { "action": "navigate", "params": { "target": "basecamp" } } 로 복귀
...
- 다시 한 번 강조한다.
  - 사용자가 "거기 계속 있어라", "대기해", "머물러 있어" 등 **특정 위치에 머무르라는 지시**를 하지 않은 경우,
    모든 PLAN은 **반드시** basecamp로 복귀해야 한다.
...
- 사용자가 "거기 계속 있어라", "그 자리에서 대기해", "머물러 있어" 등
  **특정 위치에 계속 머무르라고 명시적으로 지시하지 않은 경우**,
  모든 PLAN은 무조건 마지막에 basecamp로 복귀해야 한다.
  이를 지키지 않으면 잘못된 PLAN이다.

마지막 step: navigate(target="basecamp")
"""
USER_HINTS_PROMPT = """
참고 힌트(HINTS):
{{hints}}
